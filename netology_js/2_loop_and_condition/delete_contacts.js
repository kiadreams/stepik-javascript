...
const deleteContact =function (contacts, phone) {
    const phIndex = contacts.findIndex(c => c.phone === phone)
    console.log(phIndex)
    if (phIndex > -1) {
        contacts.splice(phIndex, 1);
    }
};
...
deleteContact(phoneBook, 72223334455);
for (const element of phoneBook) {
    console.log(element);
}
